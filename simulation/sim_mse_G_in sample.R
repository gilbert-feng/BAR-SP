    ...
    for (homo in c(0, 1)) {
      ...
      for (n in n_seq) {
        ...
        for (degree in 1:n_degree) {
          ...
          if (homo == 1) {
            prefix <- paste0("results G in mse ", "gamma = ",gamma_effect, " SBM_type = ", SBM_type, "/homo/")
          } else {
            prefix <- paste0("results G in mse ", "gamma = ",gamma_effect, " SBM_type = ", SBM_type, "/hete/")
          }
          ...
          for (i in 1:rep_time) {
            ...
            temp_mse_mat[i,1] <- BAR.SP.est$in_mse
            ...
            temp_mse_mat[i,2] <- BAR.SP.est$in_mse
            ...
            temp_mse_mat[i,3] <- SP.est$in_mse
            cat("Simulation process (SP)", round(100 * i / rep_time), "% complete!\n")
            
            # (4) OLS
            start_time <- Sys.time()
            OLS.est <- OLSCV(X,Y,homo)
            temp_time_mat[i,4] <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
            temp_mse_mat[i,4] <- OLS.est$in_mse
            cat("Simulation process (OLS)", round(100 * i / rep_time), "% complete!\n")
            
            # (5) RNC
            start_time <- Sys.time()
            RNC.est <- RNCCV(X,Y,A,gamma=0.1,K_fold=K_fold)
            temp_time_mat[i,5] <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
            temp_mse_mat[i,5] <- RNC.est$in_mse
            cat("Simulation process (RNC)", round(100 * i / rep_time), "% complete!\n")
            
            # (6) SIM
            start_time <- Sys.time()
            SIM.est <- SIMCV(X,Y,A,homo)
            temp_time_mat[i,6] <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
            temp_mse_mat[i,6] <- SIM.est$in_mse
            cat("Simulation process (SIM)", round(100 * i / rep_time), "% complete!\n")
