...
zero_num <- 2
temp_zero <- array(0, dim = c(rep_time, 2, 2))
zero_mat <- matrix(0, length(n_seq)*zero_num, 6)
sd_ratio_table <- array(0, dim = c(length(n_seq), n_degree * 3, 2))
true_projection <- matrix(0,5,n_degree)
    ...
    for (homo in c(0, 1)) {
      ...
      for (n in n_seq) {
        ...
        for (degree in 1:n_degree) {
          ...
          # Calculate true projection components (25.10.31)
          SP.true.proj <- SP.Prod(X,ProdSVD.fit$x_proj,ProdSVD.fit$Z_hat,EY,ProdSVD.fit$W_cup,K,ProdSVD.fit$r,homo,sig_level)
          ...
          if (homo == 1) {
            prefix <- paste0("results ", "gamma = ",gamma_effect, " SBM_type = ", SBM_type, "/homo/")
          } else {
            prefix <- paste0("results ", "gamma = ",gamma_effect, " SBM_type = ", SBM_type, "/hete/")
          }
          ...
          true_projection[,degree] <- SP.true.proj$project_measures
          # Save results
        ...
        write.csv(true_projection, paste0(prefix, "n = ", n, "/true_projection.csv"))
