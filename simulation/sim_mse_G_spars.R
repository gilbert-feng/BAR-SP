...
mse_mat <- matrix(0,length(s_0_seq),3)
    ...
    for (n in n_seq) {
      for (degree in 1:n_degree) {
        ...
        for (s_ind in 1:length(s_0_seq)) {
          ...
          if (homo == 1) {
            prefix <- paste0("results G"," mse spars","/homo/")
          } else {
            prefix <- paste0("results G"," mse spars","/hete/")
          }
          ...
          mse_mat[s_ind,degree] <- mean(temp_mse_mat[,1]/temp_mse_mat[,2])
